...
public class ConsultarCarga {
    private static ConsultarCarga instance;

    private Carga veiculoAtivo;

    private JFrame tela = new JFrame("Consultar / excluir carga");
...
//        line++;
        ...
        btExcluir.addActionListener(e -> {
            ...
            switch (y) {
                case 0:
                    ...
                    if (BDVeiculos.getBDCarga().contains(veiculo)) {
                        BDVeiculos.getBDCarga().remove(veiculo);
                        tela.dispose();
                    }
                    tela.dispose();
                    break;
    ...
    private boolean consultar() {
        ...
        Optional<Carga> veicOpt = BDVeiculos.getBDCarga().stream().filter(s -> s.getPlaca().equals(placa)).findAny();
        ...
        try {
            ...
            txtTara.setText(String.valueOf(veiculo.getTara()));
            txtCargaMaxima.setText(String.valueOf(veiculo.getCargaMax()));
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    private void make_editable(boolean editable) {
        ...
        txtTara.setEditable(editable);
        txtCargaMaxima.setEditable(editable);
    }
}
